...
export default function SuratShow() {
    ...
    useEffect(() => {
        if (initialPos.page) setPageNumber(initialPos.page);
    }, [initialPos.page]);

    const updateBadgePixels = useCallback((dispW: number, dispH: number) => {
        if (dispW === 0 || dispH === 0) return;
        setBadgeState({
            x: data.x * dispW,
            y: data.y * dispH,
            width: data.width * dispW,
            height: data.height * dispH
        });
    }, [data]);
    ...
    const handleResizeStop = (_e: any, _direction: any, ref: HTMLElement, _delta: any, position: { x: number, y: number }) => {
        ...
        setData({ page: pageNumber, x: newX, y: newY, width: newW, height: newH });
    ...
    const handleSavePlacement = () => {
        put(route('sekretaris.surat.placement.update', surat.id), { preserveScroll: true });
    };
    ...
    const handleSubmit = () => {
        ...
        router.post(
            route('sekretaris.surat.submit', surat.id),
            {},
            {
                onFinish: () => {
    ...
    return (
        <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f0f4ff 0%, #fafafa 100%)' }}>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                {/* ── Status bar ── */}
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        {canSubmit && (
                            <button
                                ...
                                className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl shadow-sm transition-all duration-200 active:scale-95 disabled:opacity-50"
                            >
                        ...
                        {canSubmit && (
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white rounded-xl shadow-md transition-all duration-200 active:scale-95 ${!surat.qr_position ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-lg'}`}
                        ...
                        {isWaiting && (
                            <span className="text-sm text-amber-600 bg-amber-50 border border-amber-200 px-4 py-2 rounded-xl">
                                ⏳ Menunggu keputusan approver
                            </span>
                        )}
                        {isApproved && (
                            <span className="text-sm text-emerald-600 bg-emerald-50 border border-emerald-200 px-4 py-2 rounded-xl">
                            ...
                            >
                                ...
                                {renderedWidth > 0 && renderedHeight > 0 && pageNumber === data.page && (
                                    <Rnd
                                        ...
                                        className={`absolute z-10 rounded shadow-md bg-white/90 overflow-hidden ${
                                            canSubmit 
